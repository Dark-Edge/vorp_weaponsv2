...
RegisterServerEvent("syn_weapons:addcomp", function(weaponid, added)
	...
	--print("serverside: ", weaponid, added)
	--print("serverside2: ", components, added)
    if weaponid then
        exports.oxmysql:execute('UPDATE loadout Set comps = @comps WHERE id = @id', { ['id'] = weaponid, ['comps'] = components }, function(result)
			--print('Updated ',table.concat(result,", "),' rows')
		end)
    end
end)
