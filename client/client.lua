...
AddEventHandler("syn_weapons:applymods", function()
	...
	--print("clientside: ",weaponid, added)
...
local function createobject(x, y, z, objecthash)
	if not createdobject then
		...
		PointCamAtEntity(cam, wepobject, 0.0, 0.0, 0.0, true) -- the camera is targetting the current selected weapon
	...
	while true do
		...
		elseif WarMenu.IsMenuOpened('shop') then
			...
			if WarMenu.Button(Config2.Language.exitmenu) then
				...
				currentshop = nil
				WarMenu.CloseMenu()
				EndCam() -- function to drop all cameras setup, taken from redemrp_shops
				GetJob = false
			end
